...
### Stato attuale (2026-04-24) — v2.2.8
...
| v2.2.7 | `87aefda` | "Genera Export" scrive anche su file (aggiunge `SaveToFile()`) — prima salvava solo nel campo testo; fix lfs.mkdir: rimuove slash finale; fallback `os.execute mkdir -p`; log mkdir per diagnostica |
| v2.2.8 | — | Log versione+path all'avvio (`Initialize v2.2.8 path=...`); `SaveToFile` fallback a `media/agendas.json` se sottocartella non scrivibile; log mkdir dettagliato |
